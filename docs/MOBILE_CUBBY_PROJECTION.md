...
## 2.1.0-cubby.2 host compatibility behavior

The per-Cubby `brctl serve` manager now resolves a host-native executable. On Windows it prefers `brctl.exe`; on Linux/macOS it requires a native `brctl` file and will **not** attempt to execute the bundled PE executable. Projection/status remains available and reports the host limitation explicitly. APDU proxy calls return service-unavailable semantics until a compatible binary is materialized.
