...
test('worker pong permits backend-aware memory telemetry without requiring per-thread RSS', () => {
  const rec = { t: 'pong', nonce: 'abcdefghijklmnop', busy: false, usage: { vfsBytes: 1, vfsNodes: 1, historyBytes: 0, pendingOutBytes: 0, overflows: 0, memory: { backend: 'thread', heapUsedBytes: 123, heapTotalBytes: 456, externalBytes: 7, arrayBuffersBytes: 8, rssScope: 'shared-gateway-process (no per-thread RSS)' } } };
  assert.doesNotThrow(() => encodeRecord(rec, 'worker_to_gateway'));
});
