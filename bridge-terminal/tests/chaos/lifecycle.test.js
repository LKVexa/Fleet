...
for (const backend of ['thread', 'process']) test(`[${backend}] non-cooperative CPU loop (catastrophic RegExp) cannot be cancelled in-process: the supervisor terminates only that worker; the other tenant is unaffected`, async () => {
  const stallMs = backend === 'process' ? 3000 : 1200;
  const g = await startGateway({ workerPingMs: 200, workerStallMs: stallMs, workerBackend: backend });
  try {
    ...
    assert.strictEqual(a.type('session.exit')[0].payload.reason, 'worker_failure'); assert.ok(ms < 8000, `contained in ${ms} ms`);
    if (pid) { for (let i = 0; i < 40 && alive(pid); i++) await sleep(50); assert.strictEqual(alive(pid), false, 'process worker must be reaped'); }
