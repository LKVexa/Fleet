...
function readConfig(env) {
  ...
  return {
    backend: env.VWS_W_BACKEND === 'process' ? 'process' : 'thread',
...
function createWorkerSession({ env, emit, onFinished, diag }) {
  ...
  function usage() {
    ...
    // V8 heap/external/ArrayBuffer counters are meaningful for the current worker isolate.
    // RSS is process-wide in a Worker thread, so exposing it as a per-worker number would be false precision.
    const memory = {
      backend: cfg.backend,
      heapUsedBytes: m.heapUsed, heapTotalBytes: m.heapTotal, externalBytes: m.external, arrayBuffersBytes: m.arrayBuffers,
      rssScope: cfg.backend === 'process' ? 'worker-process' : 'shared-gateway-process (no per-thread RSS)'
    };
    if (cfg.backend === 'process') memory.rssBytes = m.rss;
    return { vfsBytes: v.bytes, vfsNodes: v.nodes, historyBytes: hist, pendingOutBytes: gate.pendingBytes, overflows: gate.stats.overflows, memory };
  }
