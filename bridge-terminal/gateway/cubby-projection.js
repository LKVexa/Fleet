...
async function ensureServeForCubby(root, cubbyId) {
  ...
  try {
    ...
    return {
      ...
      host_supported: e && e.code === 'EHOSTBIN' ? false : undefined,
      cubby_id: cubbyId,
...
function projectionHtml(cubby, session, port, gateResult, serveInfo) {
  ...
  const serveUnavailable = !!(serveInfo && (serveInfo.error || serveInfo.host_supported === false));
  const serveError = serveInfo && (serveInfo.error || serveInfo.host_reason) || '';
  ...
  <dl>
    ...
    <dt>host support</dt><dd><code>${esc(serveUnavailable ? 'unavailable' : 'available')}</code>${serveError ? ' — ' + esc(serveError) : ''}</dd>
...
<main>
  ...
  ${serveReady ? '<span class="tag ok">brctl serve · ready</span>' : (mobile ? (serveUnavailable ? '<span class="tag warn-tag">brctl serve · unavailable on host</span>' : '<span class="tag warn-tag">brctl serve · starting</span>') : '')}
  <h1>Cubby ${esc(id)}</h1>
  ...
  if (serveRoute) {
    ...
    if (serveRoute.kind === "apdu") {
      ...
      readBody(req)
        ...
        .catch((e) => json(res, (e && (e.code === 'EHOSTBIN' || e.code === 'ENOENT' || e.code === 'EREADYTIMEOUT')) ? 503 : 400, { error: "apdu_failed", code: e && e.code || null, reason: String(e && e.message) }));
      return true;
    }
  }
