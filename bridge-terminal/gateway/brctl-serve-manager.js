...
function resolveBrctlInfo(root, platform = process.platform) {
  const win = path.join(root, REL_BRCTL_WIN);
  const native = path.join(root, REL_BRCTL);
  const hasWin = fs.existsSync(win), hasNative = fs.existsSync(native);
  if (platform === 'win32') {
    if (hasWin) return { path: win, supported: true, platform, reason: null };
    if (hasNative) return { path: native, supported: true, platform, reason: null };
    return { path: null, supported: false, platform, reason: 'brctl binary missing' };
  }
  if (hasNative) return { path: native, supported: true, platform, reason: null };
  if (hasWin) return { path: win, supported: false, platform, reason: `only Windows brctl.exe is bundled; no native brctl for ${platform}` };
  return { path: null, supported: false, platform, reason: 'brctl binary missing' };
}
function resolveBrctl(root) {
  const i = resolveBrctlInfo(root);
  return i.supported ? i.path : null;
}
...
class CubbyServe extends EventEmitter {
  constructor(cubbyId, root) {
    ...
    this.root = root;
    this.brctlInfo = resolveBrctlInfo(root);
    this.brctl = this.brctlInfo.supported ? this.brctlInfo.path : null;
  ...
  info() {
    return {
      ...
      brctl: this.brctl,
      host_platform: this.brctlInfo.platform,
      host_supported: this.brctlInfo.supported,
      host_reason: this.brctlInfo.reason,
  ...
  start() {
    ...
    if (!this.brctl) {
      this.lastError = this.brctlInfo.reason || 'brctl unavailable';
      const e = new Error(this.lastError); e.code = this.brctlInfo.path ? 'EHOSTBIN' : 'ENOENT';
    ...
    return new Promise((resolve, reject) => {
      ...
      const failTimer = setTimeout(() => {
        ...
        const e = new Error('brctl serve READY timeout'); e.code = 'EREADYTIMEOUT'; reject(e);
      }, 8000);
  ...
  apdu(hex) {
    ...
    if (this.queue.length >= 64) { const e = new Error('APDU queue full'); e.code = 'EQUEUEFULL'; return Promise.reject(e); }
    return new Promise((resolve, reject) => {
      const job = {
        hex: n.hex,
        resolve,
